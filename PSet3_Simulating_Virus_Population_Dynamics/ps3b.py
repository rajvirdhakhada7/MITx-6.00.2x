...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try:
                newChild = virus.reproduce(self.popDensity)
                newChilds.append(newChild)
            except NoChildException:
                continue
        self.viruses.extend(newChilds)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    timesteps[:] = [value / numTrials for value in timesteps]
    
    pylab.plot(timesteps, label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        resistantScore = sum(self.resistances[drug] for drug in activeDrugs)

        if resistantScore == len(activeDrugs) and \
            random.random() <= self.maxBirthProb * (1 - popDensity):

            childResistance = {k : (not v if random.random()<= self.mutProb
                                           else v)
                              for k, v in self.resistances.items()}
        
            return ResistantVirus(self.maxBirthProb, self.clearProb, childResistance,
                                    self.mutProb)
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        """
        if newDrug not in self.preDrugs:
            self.preDrugs.append(newDrug)
    ...
    def getResistPop(self, drugResist):
        ...
        """
        resistViruses =  [virus for virus in self.viruses \
            if sum(virus.isResistantTo(drug) for drug in drugResist) == len(drugResist)]
        return len(resistViruses)
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try:
                newChild = virus.reproduce(self.popDensity, self.preDrugs)
                newChilds.append(newChild)
            except NoChildException:
                continue
        self.viruses.extend(newChilds)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    #viruses = [ResistantVirus(maxBirthProb, clearProb, \
                        #resistances, mutProb) for _ in range(numViruses)]
    times = 300
    timeSteps = [0] * times
    resTimeSteps = [0] * times
    for i in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb, \
                        resistances, mutProb) for _ in range(numViruses)]
        ...
        for t in range(times):
            ...
            timeSteps[t] += patient.update()
            resTimeSteps[t] += patient.getResistPop(['guttagonol'])
    timeSteps[:] = [value / numTrials for value in timeSteps]
    resTimeSteps[:] = [value / numTrials for value in resTimeSteps]

    pylab.plot(timeSteps, label = "AverageVirusPop")
    pylab.plot(resTimeSteps, label = "ResistantVirusPop")
...
simulationWithDrug2(75, 100, .8, 0.1, {"guttagonol": True}, 0.8, 1)